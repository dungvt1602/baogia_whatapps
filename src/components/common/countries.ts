// Danh sách quốc gia in cứng: tên (EN), mã ISO2, mã điện thoại (dial, không có dấu +).
// Cờ được suy ra từ ISO2 (regional indicator) nên không cần lưu.
...
// Cờ emoji suy từ ISO2 (2 ký tự A-Z -> regional indicator symbols).
...
// Bí danh phổ biến -> ISO2, giúp chuẩn hoá dữ liệu cũ ("KOREA", "USA", tên tiếng Việt...).
const ALIASES: Record<string, string> = {
  korea: "KR", "south korea": "KR", "north korea": "KP",
  usa: "US", us: "US", america: "US",
  uk: "GB", england: "GB", britain: "GB",
  uae: "AE",
  "việt nam": "VN", "viet nam": "VN",
  "ấn độ": "IN", "an do": "IN",
  "nhật bản": "JP", "nhat ban": "JP", "nhật": "JP",
  "hàn quốc": "KR", "han quoc": "KR",
  "trung quốc": "CN", "trung quoc": "CN",
  "thái lan": "TH", "thai lan": "TH",
  "mỹ": "US", "hoa kỳ": "US", "hoa ky": "US",
  anh: "GB", "đức": "DE", duc: "DE", "pháp": "FR", phap: "FR", nga: "RU", "úc": "AU",
};

export function findCountry(nameOrIso: string): Country | undefined {
  const v = (nameOrIso || "").trim().toLowerCase();
  if (!v) return undefined;
  const direct = COUNTRIES.find((c) => c.name.toLowerCase() === v || c.iso2.toLowerCase() === v);
  if (direct) return direct;
  const iso = ALIASES[v];
  return iso ? COUNTRIES.find((c) => c.iso2 === iso) : undefined;
}

// Gắn mã vùng mới vào số điện thoại: bỏ mã quốc gia cũ (nếu nhận ra) rồi thêm dial mới.
const DIALS_DESC = [...new Set(COUNTRIES.map((c) => c.dial))].sort((a, b) => b.length - a.length);
export function applyDial(phone: string, dial: string): string {
  let local = (phone || "").replace(/\D/g, "");
  for (const d of DIALS_DESC) {
    if (local.startsWith(d)) { local = local.slice(d.length); break; }
  }
  return local ? dial + local : ""; // chưa có số cục bộ -> để trống (không lưu mỗi mã vùng)
}

// Tách số thành { mã vùng, số cục bộ }. Ưu tiên mã vùng của quốc gia (market) nếu số khớp.
export function splitPhone(phone: string | null | undefined, market?: string | null): { dial: string; local: string } {
  const digits = (phone || "").replace(/\D/g, "");
  if (!digits) return { dial: "", local: "" };
  const c = market ? findCountry(market) : undefined;
  if (c && digits.startsWith(c.dial)) return { dial: c.dial, local: digits.slice(c.dial.length) };
  for (const d of DIALS_DESC) {
