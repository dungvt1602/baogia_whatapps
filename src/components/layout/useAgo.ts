...
export function useAgo() {
  ...
  const navDef = [
    ...
    ...(isAdmin
      ? [
          ...
          { key: "receiveChannels", label: "Kênh nhận", icon: "📥" },
  ...
  const titles: Record<string, [string, string]> = {
    ...
    zalo: ["Zalo OA", "Kết nối Official Account — token tự làm mới, webhook nhận tin"],
  ...
  return {
    ...
    newBtnStyle:
      tiny || ["channels", "receiveChannels", "zalo", "logs", "sendjobs", "inbox", "send", "products", "customers", "team"].includes(page) || (page === "tpl" && !st.tplQuoteId)
        ? "display:none"
