...
import type { AgoState } from "./state";
import {
  QUOTES,
  STATUS,
  STATS,
...
export function useAgo() {
  ...
  const go = (page: string) => patch({ page, navOpen: false, sentMsg: "" });
  ...
  const decorate = (q: Quote) => {
    ...
    return {
      ...
      statusColor: s.color,
      statusBg: s.bg,
      open: () => patch({ page: "template", quoteId: q.id, sentMsg: "" }),
    };
  };
  ...
  const navDef = [
    ...
    { key: "new", label: "Tạo báo giá", icon: "✎", badge: "" },
  ...
  return {
    ...
    // ---- auth ----
    isAuth: st.screen === "login" || st.screen === "register",
    showLogin: st.screen === "login",
    showRegister: st.screen === "register",
    isApp: st.screen === "app",
    passType: st.showPass ? "text" : "password",
    eyeIcon: st.showPass ? "🙈" : "👁",
    togglePass: () => patch({ showPass: !st.showPass }),
    toggleRemember: () => patch({ remember: !st.remember }),
    doLogin: () => {
      if (!st.email || !st.password) return patch({ loginError: true });
      const local = st.email.toLowerCase().split("@")[0];
      const isAdm = local.includes("admin") || local.includes("giam.doc");
      const name = st.regDone && st.regName ? st.regName : isAdm ? "Trần Quốc Bảo" : "Lê Ngọc Anh";
      const parts = name.trim().split(/\s+/);
      const ini = (parts.length > 1 ? parts[parts.length - 2][0] + parts[parts.length - 1][0] : name.slice(0, 2)).toUpperCase();
      patch({ screen: "app", page: "dash", loginError: false, role: isAdm ? "admin" : "staff", userName: name, userInitials: ini });
    },
    goRegister: (e: React.MouseEvent) => {
      e.preventDefault();
      patch({ screen: "register", regError: "", regDone: false });
    },
    goLogin: (e: React.MouseEvent) => {
      e.preventDefault();
      patch({ screen: "login", loginError: false });
    },
    onEmail: (e: React.ChangeEvent<HTMLInputElement>) => patch({ email: e.target.value }),
    onPassword: (e: React.ChangeEvent<HTMLInputElement>) => patch({ password: e.target.value }),
    onRegName: (e: React.ChangeEvent<HTMLInputElement>) => patch({ regName: e.target.value }),
    onRegEmail: (e: React.ChangeEvent<HTMLInputElement>) => patch({ regEmail: e.target.value }),
    onRegPass: (e: React.ChangeEvent<HTMLInputElement>) => patch({ regPass: e.target.value }),
    onRegPass2: (e: React.ChangeEvent<HTMLInputElement>) => patch({ regPass2: e.target.value }),
    doRegister: () => {
      if (!st.regName || !st.regEmail || !st.regPass) return patch({ regError: "Vui lòng điền đầy đủ thông tin.", regDone: false });
      if (st.regPass.length < 8) return patch({ regError: "Mật khẩu tối thiểu 8 ký tự.", regDone: false });
      if (st.regPass !== st.regPass2) return patch({ regError: "Mật khẩu nhập lại không khớp.", regDone: false });
      patch({ regError: "", regDone: true, email: st.regEmail, password: st.regPass });
    },
    logout: () => patch({ screen: "login", navOpen: false }),
    ...
    userName: st.userName || "Lê Ngọc Anh",
    userInitials: st.userInitials || "NA",
    ...
    newBtnStyle:
      tiny || ["channels", "logs", "send"].includes(page) || (page === "tpl" && !st.tplQuoteId)
        ? "display:none"
    ...
    tplList: (st.quoteTpls[st.tplQuoteId] || []).map((t) => ({
      ...
      dupTpl: () =>
        patchFn((s) => {
          ...
          return { tplMenu: "", quoteTpls: { ...s.quoteTpls, [st.tplQuoteId]: [...list, { ...t, id: "t" + Date.now(), name: t.name + " (bản sao)", updated: "Vừa xong", by: s.userName || "Bạn", sends: 0 }] } };
        }),
    ...
    saveTpl: () =>
      patchFn((s) => {
        ...
        const next = f.id
          ...
          : [...list, { id: "t" + Date.now(), name: f.name, icon: f.icon, content: f.content, updated: "Vừa xong", by: s.userName || "Bạn", sends: 0 }];
