...
import { sendViaChannel } from "./senders";
...
export async function previewSend(params: {
  ...
  actor?: { id?: bigint | number | null; name?: string | null };
}) {
  const template = await prisma.template.findUnique({
    where: { id: BigInt(params.templateId) },
    include: { quotation: true, channel: true, customers: true },
  });
  ...
  const sample = renderTemplate(template.body || "", template.quotation, recipients[0]);
...
export async function confirmSend(params: {
  ...
  const batch = await prisma.sendBatch.findUnique({
    ...
    include: {
      channel: true,
      template: { include: { quotation: true, channel: true, customers: true } },
    },
  });
  ...
  await prisma.sendJob.createMany({
    data: recipients.map((c) => ({
      ...
      channel: channelType,
      message: renderTemplate(batch.template.body || "", batch.template.quotation!, c),
      status: "QUEUED",
    })),
  });
...
export async function processNextBatch() {
  ...
  const batch = await prisma.sendBatch.findUnique({
    ...
    include: { channel: true, template: { include: { channel: true } } },
  });
