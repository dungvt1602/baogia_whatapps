...
import { actionLabel, actionGroup, GROUP_LABELS, type ActionGroup } from "@/components/common/activityLabels";
...
// Màu nhãn hành động theo nhóm — nhìn lướt là biết thao tác thuộc mảng nào.
const GROUP_COLORS: Record<ActionGroup, [string, string]> = {
  auth: ["#EEF2F5", "#33475B"],
  customer: ["#E7F5EC", "#1F7440"],
  template: ["#EAF0FB", "#2B4F8E"],
  quotation: ["#FFF3E0", "#8A5A00"],
  send: ["#F3ECFB", "#5B3A8E"],
  user: ["#FDECEC", "#8E2B2B"],
  channel: ["#E6F7F7", "#1B6E6E"],
  zalo: ["#E5F1FF", "#0B5ED7"],
  system: ["#F1F4F1", "#6B7A70"],
};
function ActionBadge({ code }: { code: string }) {
  const [bg, fg] = GROUP_COLORS[actionGroup(code)];
  return <span title={code} style={sx(`font-size:12px; font-weight:600; padding:2px 9px; border-radius:6px; background:${bg}; color:${fg}; white-space:nowrap`)}>{actionLabel(code)}</span>;
}
...
function ResultBadge({ r }: { r: string | null }) {
  ...
  return <span style={sx(`font-size:11px; font-weight:700; padding:2px 8px; border-radius:5px; background:${bg}; color:${fg}`)}>{ok ? "Thành công" : bad ? "Thất bại" : r || "—"}</span>;
}
...
export default function LogsScreen() {
  ...
  const view = useMemo(() => {
    ...
    const arr = rows.filter(
      (l) =>
        (!group || actionGroup(l.action) === group) &&
        (!kw || [l.actorName, l.action, actionLabel(l.action), l.target, l.note, l.result].some((v) => (v || "").toLowerCase().includes(kw))),
    );
    arr.sort((a, b) => {
  ...
  function exportCsv() {
    ...
    const data = view.map((l) => [fmtDate(l.createdAt), l.actorName || "", actionLabel(l.action), l.target || "", l.result || "", l.note || ""]);
  ...
  return (
    <div>
      ...
      <div style={sx("display:flex; align-items:center; gap:8px; margin-bottom:10px; flex-wrap:wrap")}>
        ...
        <select value={group} onChange={(e) => { setGroup(e.target.value as ActionGroup | ""); setPage(1); }} style={sx(`${inp} height:34px; width:190px`)}>
          <option value="">Tất cả loại thao tác</option>
          {(Object.keys(GROUP_LABELS) as ActionGroup[]).map((g) => <option key={g} value={g}>{GROUP_LABELS[g]}</option>)}
        </select>
        <div style={sx("font-size:12.5px; color:#7B8A80")}>{view.length} bản ghi · 30 ngày gần nhất</div>
        <div style={sx("flex:1")} />
          ...
          <tbody>
            ...
            {paged.map((l, i) => (
              <tr key={l.id} style={sx(`background:${i % 2 ? "#FBFDFB" : "#fff"}`)}>
                ...
                <td style={sx(gtd)}><ActionBadge code={l.action} /></td>
      ...
      <div style={sx("font-size:11.5px; color:#8B9A90; margin-top:8px")}>Ghi lại <b>mọi thao tác thêm / sửa / xóa</b> trong web kèm người thực hiện; sửa thì ghi rõ đổi trường nào (cột Ghi chú, bấm Xem để đọc đủ). Giữ <b>30 ngày</b>; riêng log gửi hàng loạt / trả lời tự động chỉ giữ 3 ngày. Bấm tiêu đề cột để sắp xếp.</div>
        ...
        <div style={sx("position:fixed; inset:0; z-index:70; display:flex; align-items:center; justify-content:center; padding:20px")}>
          ...
          <div style={sx("position:relative; width:100%; max-width:460px; max-height:88vh; overflow:auto; background:#fff; border-radius:20px; padding:24px; box-shadow:0 30px 70px -20px rgba(8,40,24,.5)")}>
            ...
            <div style={sx("border:1px solid #E9EEE9; border-radius:12px; overflow:hidden")}>
              {([["Thời gian", fmtDate(detail.createdAt)], ["Người thực hiện", detail.actorName || "— (hệ thống)"], ["Hành động", `${actionLabel(detail.action)} (${detail.action})`], ["Đối tượng", detail.target || "—"], ["Kết quả", detail.result === "SUCCESS" ? "Thành công" : detail.result === "FAILED" ? "Thất bại" : detail.result || "—"], ["Ghi chú", detail.note || "—"]] as [string, string][]).map(([k, v], idx) => (
