...
export default function ReceiveChannelsScreen() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setRows(await getJSON<Channel[]>("/api/receive-channels")); } catch (e) { setErr((e as Error).message); }
  ...
  function del(c: Channel) { setPending({ text: `Xóa kênh nhận "${c.name}"?`, run: async () => { await sendJSON("DELETE", `/api/receive-channels/${c.id}`); await load(); toast.success("Đã xóa kênh nhận"); } }); }
  function delSelected() {
    if (selected.size === 0) return;
    const ids = [...selected];
    setPending({ text: `Xóa ${ids.length} kênh nhận đã chọn?`, run: async () => { await Promise.all(ids.map((id) => sendJSON("DELETE", `/api/receive-channels/${id}`))); setSelected(new Set()); await load(); toast.success(`Đã xóa ${ids.length} kênh nhận`); } });
  }
  ...
  function exportCsv() {
    ...
    a.download = "kenh-nhan.csv"; a.click(); URL.revokeObjectURL(a.href);
  }
  ...
  return (
    <div>
      ...
      <div style={sx("display:flex; align-items:center; gap:8px; margin-bottom:10px; flex-wrap:wrap")}>
        ...
        <div style={sx("font-size:12.5px; color:#7B8A80")}>{view.length} kênh nhận{selected.size ? ` · chọn ${selected.size}` : ""}</div>
        <div style={sx("flex:1")} />
        ...
        <HButton s={green} onClick={() => setForm(empty())}>+ Thêm kênh nhận</HButton>
      </div>
      ...
      <div style={sx("background:#fff; border:1px solid #D3DCE3; border-radius:10px; overflow:auto; max-height:calc(100vh - 200px)")}>
        <table style={sx("width:100%; border-collapse:collapse; min-width:980px")}>
          ...
          <tbody>
            ...
            {!loading && view.length === 0 && <tr><td colSpan={9} style={sx(gtd + "; text-align:center; color:#8B9A90; padding:24px")}>Chưa có kênh nhận nào. Bấm “+ Thêm kênh nhận”.</td></tr>}
            {paged.map((c, i) => {
      ...
      <div style={sx("display:flex; align-items:center; gap:8px; margin-top:10px")}>
        <div style={sx("font-size:12.5px; color:#7B8A80; flex:1")}>Trang {curPage}/{totalPages} · {view.length} kênh nhận</div>
        ...
        <div style={sx("position:fixed; inset:0; z-index:60; display:flex; align-items:center; justify-content:center; padding:20px")}>
          ...
          <div style={sx("position:relative; width:100%; max-width:480px; max-height:88vh; overflow:auto; background:#fff; border-radius:20px; padding:24px; box-shadow:0 30px 70px -20px rgba(8,40,24,.5)")}>
            <div style={sx("display:flex; align-items:center; gap:10px; margin-bottom:16px")}>
              <div style={sx("font-size:17px; font-weight:700; color:#14261A; flex:1")}>{form.id ? "Sửa kênh nhận" : "Thêm kênh nhận"}</div>
            ...
            <div style={sx("display:flex; gap:10px; margin-top:6px")}>
              <HButton s={`${green} flex:1; height:44px`} onClick={save}>{form.id ? "Lưu thay đổi" : "Thêm kênh nhận"}</HButton>
        ...
        <div style={sx("position:fixed; inset:0; z-index:70; display:flex; align-items:center; justify-content:center; padding:20px")}>
          ...
          <div style={sx("position:relative; width:100%; max-width:440px; max-height:88vh; overflow:auto; background:#fff; border-radius:20px; padding:24px; box-shadow:0 30px 70px -20px rgba(8,40,24,.5)")}>
            <div style={sx("display:flex; align-items:center; gap:10px; margin-bottom:14px")}>
              <div style={sx("font-size:17px; font-weight:700; color:#14261A; flex:1")}>Chi tiết kênh nhận</div>
